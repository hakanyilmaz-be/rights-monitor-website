...
const KanunKamuDavasiSayisi = () => {
  ...
  const data = [
    [t('kanunb.category'), 'TCK 309-316', { role: 'annotation' }, 'TFK', { role: 'annotation' }, 'TMK', { role: 'annotation' }],
    ['', parseFloat(a2Value), a2Value, parseFloat(b2Value), b2Value, parseFloat(c2Value), c2Value],
  ];

  const options = {
    title: t('kanunb.publicProsecutionTitle'),
    chartArea: { width: '50%', height: '70%' },
    vAxis: {
      minValue: 0,
    },
    hAxis: {
      title: `${t('kanunb.totalCount')}: ${d2Value}`,
