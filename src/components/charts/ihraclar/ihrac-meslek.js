...
const IhracMeslek = () => {
    ...
    useEffect(() => {
        const fetchCSVData = async () => {
            ...
            try {
                ...
                setDataRows(parsedData.slice(12, 240)); 
            } catch (error) {
    ...
    const columns = [
        { name: t('profession'), selector: row => row[0], sortable: true, grow: 1 },
    ...
    return (
        <div>
            <p style={{ fontWeight: 'bold' }}>{t('dismissalsByProfession')}</p>
            <DataTable
                columns={columns}
                data={dataRows}
                pagination
